...
class WebhookDashboard {
    ...
    setupUI() {
        ...
        document.getElementById('webhookUrl').textContent = `${baseUrl}/api/webhook`;
    ...
    renderWebhooks() {
        ...
        if (filtered.length === 0) {
            list.innerHTML = '';
            list.appendChild(emptyState.cloneNode(true));
            return;
        }
        ...
        filtered.forEach(webhook => {
            ...
            // Time
            card.querySelector('.webhook-time').textContent = this.formatTime(webhook.receivedAt);
            
            // Headers
            card.querySelector('.headers-content').textContent = 
                JSON.stringify(webhook.headers, null, 2);
            
            // Body
            const bodyContent = card.querySelector('.body-content');
            card.querySelector('.content-type').textContent = webhook.contentType || '';
            ...
            card.querySelector('.content-length').textContent = 
